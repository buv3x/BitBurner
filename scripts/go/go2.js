...
export async function main(ns) {
  ...
  for (const [key, value] of state.grid.entries()) {
    if(value == 'X') {
      boardState = ns.go.resetBoardState("Illuminati", 13);
      parseGrid(boardState, state);
      break;
    }
  }
  ...
  outer: while(true) {
    ...
    if(state.turnCounter > 5000) {
      await(ns.prompt("Oh, shoot!"));
      return;
    }
    if(ns.go.getCurrentPlayer() == 'None') {
      let endDate = Date.now();
      // ns.toast("Total time: " + (endDate - startDate) + ". " +
      //   "AI time: " + aiTime);
      return;
    }
    let boardState = ns.go.getBoardState();
    ...
    if(state.stage == 2) {
      if(state.substage == 0) {
        ...
        if(state.baseDirection == 0) {
          checkPoint.x = state.baseStart - 1;
          checkPoint.y = state.minJ + 1;
          columnDirection = 1;
        } else if (state.baseDirection == 1) {
          checkPoint.x = state.minI + 1;
          checkPoint.y = state.baseStart - 1;
          columnDirection = 0;
        } else if (state.baseDirection == 2) {
          checkPoint.x = state.baseStart - 1;
          checkPoint.y = state.maxJ - 1;
          columnDirection = 1;
        } else {
          checkPoint.x = state.maxI - 1;
          checkPoint.y = state.baseStart - 1;
          columnDirection = 0;          
        }
        let checkCell = state.grid.get(gridKey(checkPoint.x, checkPoint.y));

        if(checkCell == 'O') {
          let columnPoint = null;
          while(state.basePoints.length > 0) {
            let columnCheckPoint = state.basePoints[0];
            state.basePoints.shift();
          ...
          if(columnPoint != null) {
            ...
            if(state.candidateUsed >= state.candidates.length) {
              ns.print("Ran out of candidates");
              state.stage = 666;
            }
            state.baseIndex = 0;
            state.primaryStack = [];
      ...
      if(state.substage == 1) {
        ...
        let columnDirection;
        if(state.baseDirection == 0) {
          checkPoint.x = state.baseEnd + 1;
          checkPoint.y = state.minJ + 1;
          columnDirection = 3;
        } else if (state.baseDirection == 1) {
          checkPoint.x = state.minI + 1;
          checkPoint.y = state.baseEnd + 1;
          columnDirection = 2;
        } else if (state.baseDirection == 2) {
          checkPoint.x = state.baseEnd + 1;
          checkPoint.y = state.maxJ - 1;
        ...
        if(checkCell == 'O') {
          let columnPoint = null;
          while(state.basePoints.length > 0) {
            let columnCheckPoint = state.basePoints[state.basePoints.length - 1];
      ...
      while(state.nextStraightPoint != null || state.primaryStack.length > 0) {
        ...
        if(state.nextStraightPoint != null) {
          ...
          if(state.grid.has(gridKey(nextPoint.x, nextPoint.y)) 
            ...
            let straightNewPoint = {};
            straightNewPoint.x = nextPoint.x;
            straightNewPoint.y = nextPoint.y;
            straightNewPoint.direction = direction;
            state.nextStraightPoint = straightNewPoint;

            let rightNewPoint = {};
            rightNewPoint.x = nextPoint.x;
            rightNewPoint.y = nextPoint.y;
            ...
            let leftNewPoint = {};
            leftNewPoint.x = nextPoint.x;
            leftNewPoint.y = nextPoint.y;
        ...
        if(state.primaryStack.length > 0) {
          ...
          if(state.grid.has(gridKey(nextPoint.x, nextPoint.y)) 
            ...
            straightNewPoint.x = nextPoint.x;
            straightNewPoint.y = nextPoint.y;
            straightNewPoint.direction = point.direction;
            state.nextStraightPoint = straightNewPoint;

            let rightNewPoint = {};
            rightNewPoint.x = nextPoint.x;
            rightNewPoint.y = nextPoint.y;
            rightNewPoint.direction = (point.direction + 1) % 4;
            rightNewPoint.baseMult = baseMult(nextPoint.x, nextPoint.y, 
              rightNewPoint.direction, state.baseDirection, state);
            ...
            leftNewPoint.x = nextPoint.x;
            leftNewPoint.y = nextPoint.y;
            leftNewPoint.direction = (point.direction + 3) % 4;
            leftNewPoint.baseMult = baseMult(nextPoint.x, nextPoint.y, 
              leftNewPoint.direction, state.baseDirection, state);
    ...
    for(let i = minCount; i <= maxCount; ++i) {
      ...
      if(streak) {
        if(cell0 != '.' || cell1 != '.') {
          ...
          if(streakLength >= 4) {
            let streakCandidate = {};
            streakCandidate.length = streakLength;
            streakCandidate.direction = k;
            streakCandidate.start = streakStart;
            streakCandidate.end = i - 1;
